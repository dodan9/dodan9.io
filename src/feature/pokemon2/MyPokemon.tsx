...
import loadingBall from "./img/pokemonBall.png";
...
const MyPokemon = () => {
  ...
  const BoxPagenation = useEffectEvent(() => {
  ...
  const MovePageEffect = useEffectEvent((dirention: number) => {
    MovePage(dirention);
  });

  const maxEffect = useEffectEvent(() => {
  ...
  const boxEffect = useEffectEvent(() => {
    if (boxIndex >= maxIndex) setIsNext(false);
    else setIsNext(true);
    setSelectedPokemon(null);
  });
  ...
  const pokemonListEffect = useEffectEvent(() => {
  ...
  const imageLoadingEffect = useEffectEvent(() => {
